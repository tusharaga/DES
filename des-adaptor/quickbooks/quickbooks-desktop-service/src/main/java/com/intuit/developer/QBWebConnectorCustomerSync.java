...
import com.emob.des.rest.model.QBWrapper;
 ...
 public class QBWebConnectorCustomerSync
   implements QBWebConnectorSvcSoap
 {
   private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(QBWebConnectorCustomerSync.class.getName());
   private DesClientInterface desClient;
   private IntegrationDetail detail;
   QBWrapper wrapper = null;
   org.slf4j.Logger logger = LoggerFactory.getLogger(QBWebConnectorCustomerSync.class);
   ...
   public String sendRequestXML(String ticket, String strHCPResponse, String strCompanyFileName, String qbXMLCountry, int qbXMLMajorVers, int qbXMLMinorVers)
   {
     ...
     String qbDate = "";
     if(detail != null){
    	 if(detail.getLast_sync_time().indexOf("T") != -1){
    		 qbDate = convertToQBDateFormat(detail.getLast_sync_time());
    	 }
    	 else{
    		 qbDate = detail.getLast_sync_time();
    	 }
    	 _return = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\r\n<?qbxml version=\"13.0\"?>\r\n<QBXML>\r\n	<QBXMLMsgsRq onError=\"stopOnError\">\r"
    	 		+ "		<CustomerQueryRq requestID=\"1\">\r\n			<FromModifiedDate >"+qbDate+"</FromModifiedDate>\r\n		</CustomerQueryRq>\r\n	</QBXMLMsgsRq>\r\n</QBXML>";
     }else{
    	 logger.info("Integeration Details is Null");
     }
     ...
     try
     {
       if (detail != null){
           ...
           try
           {
             ...
             {
               String emobXslPath = "";
               emobXslPath = prop.getProperty("generic-qbcustomer-sync");
   ...
   public ArrayOfString authenticate(String strUserName, String strPassword)
   {
     ...
     this.logger.debug(strUserName);
     this.logger.debug(strPassword);
   ...
   private String convertToQBDateFormat(String date){
	   logger.info("emob Date : " + date);
	   String qbDate = date.substring(0, 10) + "T" + date.substring(11,19);
	   logger.info("qb Date : " + qbDate);
	   return qbDate;
   }
